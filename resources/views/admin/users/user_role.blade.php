...
@section('content')
    <div class="main-panel">
        <div class="content-wrapper">
            <div class="card-body">
                <h4 class="card-title col-10">Rôles de {{ $user->firstname }} {{ $user->lastname }}</h4>
                ...
                <div class=" container row mt-5 ml-5 mr-5">
                    @foreach ($roles as $role)
                        <div class="form-group mr-4">
                            ...
                            {{-- value={{ $role->id }}> --}}
                            <label class="switch">
                                <input type="checkbox" value={{ $role->id }} {{ $role->exist ? 'checked' : '' }}
                                    onclick="attribuer()" id="check">
                                <span class="slider round"></span>
                            </label>
                            <span class="text-center mt-4 ml-2"> {{ $role->name }} </span>
                            {{-- <label for="role">{{ $role->name }} --}}
                        </div>
                    @endforeach
                </div>
                <form method="POST" action="{{ route('affecter') }}">
                    @csrf
